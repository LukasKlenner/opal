...
class IntraProceduralAliasAnalysis(final val project: SomeProject) extends AllocationSiteAndTacBasedAliasAnalysis {
    ...
    private[this] def handleUVar(uVar: AliasUVar, tac: Tac)(
        ...
        uVar.persistantUVar.defSites.foreach(pc => {
...
object EagerFieldAccessIntraProceduralAliasAnalysisScheduler extends IntraProceduralAliasAnalysisScheduler
    with EagerFieldAccessAliasAnalysisScheduler {

    override def createAnalysis(p: SomeProject): TacBasedAliasAnalysis = new IntraProceduralAliasAnalysis(p)
}

object EagerMethodInvocationIntraProceduralAliasAnalysisScheduler extends IntraProceduralAliasAnalysisScheduler
